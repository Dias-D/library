...
class Auth_Login_Customauth extends \Auth\Auth_Login_Driver {
    /*
    ...
    public function validate_user($username_or_email = '', $password = '')
    {
        ...
        $username = trim($username_or_email) ?: trim(\Input::post(\Config::get('ormauth.username_post_key', 'username'))); //  can be social security or email
        ...
        $user = \Model\Auth_User::query()
                ...
                ->where_close()
                ->and_where_open()
                ->where('deleted', null, \DB::expr('IS NULL'))
                ->and_where_close()
    ...
    public function login($username_or_email = '', $password = '')
    {
        ...
        \Session::set('useremail', $this->user->email);
    ...
    /**
     * Generates new random password, sets it for the given useremail and returns the new password.
    ...
    /**
     * Return if user is a supplier
     *
     * @return  Bool  User is a client
     */
